class Juego {
  ...
  void draw() {
    ...
    if (menu) {
      ...
      text("Presiona enter para jugar\nMovete con WASD y dispara con el mouse\n¡Que no te atrapen!", width/2, height/2);
      pop();
    } else if (perder) {
      push();
      resultado.draw(lose, true, "¡Que lastima! Has perdido");
      image(robotito2, 0, 100);
      fill(255);
      textSize(30);
      text("Pulsa click\npara reintentar", width/2, 490);
      fill(255, 100);
      rect(200, 450, 400, 90, 10);
      pop();
    } else if (ganar) {
      push();
      resultado.draw(win, true, "Vencerlos fue pan comido, ahora TL-WN es el lider de la mafia local");
      image(robotito, 0, 100);
      pop();
    } else {
